    ...
    public class DbMaintainer : IDbMaintainer
    {
        ...
        public void OnUpdatedModel<TKey>(IAuditable updatedModel)
        {
            ...
            var idMemberMaps = referenceMemberMaps
                .Select(mm =>
                {
                    /* Find first parent member map generated by an entity model map schema.
                     * It must exist, because original member maps was a refedenced member */
                    while (!mm.ModelMapSchema.IsEntity)
                        mm = mm.ParentMemberMap!;
                    return mm.ModelMapSchema.IdMemberMap!;
                })
                .Distinct();
